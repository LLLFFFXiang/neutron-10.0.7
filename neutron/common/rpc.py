...
# class neutron_service.Servcie的父类是class n_rpc.Service
# 子类与父类的class name取名相同，需要靠函数文件名（含文件目录）来区分
# class n_rpc.Service的start函数
...
class Service(service.Service):
    ...
    def start(self):
        ...
        # 一下代码的功能是：创建RPC Consumer，并在协程中运行
        self.conn = create_connection()
        LOG.debug("Creating Consumer connection for Service %s",
                  self.topic)
        # 重点是endpoints，就是处理RPC消息的对象实例，self.manager,
        # 也就是前面介绍的class L3NATAgentWithStateReport的实例对象
