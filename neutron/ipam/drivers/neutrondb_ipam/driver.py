...
class NeutronDbSubnet(ipam_base.Subnet):
    ...
    def allocate(self, address_request):
        ...
        '''
            如果上次的参数中包含IP地址,那么就做一个校验,如果校验通过,则这个IP地址就
            可以分配给对应的Port.校验算法:
            (1)校验这个IP地址,是否已经被分配了(已经存在于ipamallocations表中)
            (2)校验这个IP地址,是否是传入参数的Subnet的一个合法IP地址
        '''
        if isinstance(address_request, ipam_req.SpecificAddressRequest):
            ...
            '''
                如果传入参数中没有IP地址,那么要生成一个IP地址.生成算法是:
                (1)构建候选IP列表:这个IP地址在ipamallocationpools定义的范围内,并且
                没有被分配过(没有位于ipamallocations表中)
                (2)在这些候选IP列表中随机选择一个
            '''
        ...
        try:
            with self._context.session.begin(subtransactions=True):
                ...
                '''
                    将上述IP地址(或者是传入,或者是生成)存入ipamallocations表中,
                    表示这个IP地址已经被分配了
                '''
