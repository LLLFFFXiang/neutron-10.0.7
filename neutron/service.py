...
class RpcWorker(neutron_worker.NeutronWorker):
    ...
    # start_rpc_listeners, Plugin创建RPC Consumer的一个接口
    ...
    # class RpcWorker最重要的函数就是start函数
    def start(self):
        super(RpcWorker, self).start()
        # 查找每一个Plugin
        for plugin in self._plugins:
            # 只要这个Plugin具有start_rpc_listeners接口
            if hasattr(plugin, self.start_listeners_method):
                try:
                    # 那么就执行这个接口,也就是说,这个Plugin创建了RPC Consumer
...
class RpcReportsWorker(RpcWorker):
    # start_rpc_state_reposts_listener, Plugin创建RPC Consumer的一个接口
...
def _get_rpc_workers():
    ...
    # rpc_workers就是一个数组
    # 它的第一个元素就是RpcWorker实例
    # RpcWorker(service_plugins,worker_process_count)就是创建一个RpcWorker实例
    rpc_workers = [RpcWorker(service_plugins,
                             worker_process_count=cfg.CONF.rpc_workers)]

    if (cfg.CONF.rpc_state_report_workers > 0 and
            plugin.rpc_state_report_workers_supported()):
        # rpc_workers增加第一个元素:RpcReportsWorker的实例
        # RpcReportsWorker([plugin],worker_process_count)就是创建一个
        # RpcReportsWorker的实例
        rpc_workers.append(
            RpcReportsWorker(
                [plugin],
...
def _get_plugins_workers():
    ...
    # ServicePluginBase
    # 如果Plugin具备"get_workers"接口,那么就执行这个接口,获取"workers"
    # 并存储在plugin_worker
    return [
        plugin_worker
...
# 这里的workers = _get_rpc+workers() + _get_plugins_workers()
def _start_workers(workers):
    # 过滤出worker_process_count > 0的workers,存储在process_workers数组中
    process_workers = [
    ...
    try:
        # 如果process_workers数组中有内容
        # 即存在worker_process_count > 0 的workers
        if process_workers:
            # 那么创建ProcessLauncher(进程启动器)
            ...
            # 针对process_workers中的每一个worker,以新起进程的方式启动它
            for worker in process_workers:
                worker_launcher.launch_service(worker,
                                               worker.worker_process_count)
        else:
            # 否则的话,就用ServiceLauncher来气筒每一个worker
            # 也就是当前的进程中,以协程的方式启动
...
def _get_api_workers():
    # 首先从配置文件读取所配置的workers的值
    workers = cfg.CONF.api_workers
    # 如果配置文件没有配置，那么就以当前运行所在机器的CPU核数作为workers的值
    if workers is None:
...
def run_wsgi_app(app):
    ...
    # 这里调用的函数，就是class wsgi.Server的start函数
    # workers是调用函数_get_api_workers()获得的
...
class Service(n_rpc.Service):
    ...
    def __init__(self, host, binary, topic, manager, report_interval=None,
        ...
        self.binary = binary
        # 如下三句话，创建了self.manager，就是
        # class L3NATAgentWithStateReport的对象实例
