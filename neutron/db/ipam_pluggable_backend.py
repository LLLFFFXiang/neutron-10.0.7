...
# ML2 Plugin调用的是class IpamPluggableBackend
# allocate_ips_for_port_and_store进行Port的IP地址分配
# 这也体现了Neutron使用了IPAM对IP地址的管理
class IpamPluggableBackend(ipam_backend_mixin.IpamBackendMixin):
    ...
    def _ipam_allocate_ips(self, context, ipam_driver, port, ips,
        ...
        try:
            # 针对ips中的每一个元素,调用ipam_allocator.allocate函数进行ip地址分配
            for ip in ips:
    ...
    def _allocate_ips_for_port(self, context, port):
        ...
        """
        # port['post']是创建Port时传入的参数
        p = port['port']
        # 判断创建Port时是否传入了'fixed_ips'参数.fixed_ips是一个数组
        # 每一个元素由两个字段组成:ip_address subnet_id
        ...
        v4, v6_stateful, v6_stateless = self._classify_subnets(
            context, subnets)
        # 如果传入了'fixed_ips'参数,则对'fixed_ips'进行合法性判断
        # self._test_fixed_ips_for_port函数比较复杂
        if fixed_configured:
            # 如果fixed_ips合法,ips实际上就等于fixed_ips
            ips = self._test_fixed_ips_for_port(context,
                                                p["network_id"],
                                                p['fixed_ips'],
                                                p['device_owner'],
                                                subnets)
        # 如果没有传入'fixed_ips'参数,则根据关联的Subnet构建ips
        else:
            ips = []
    ...
    def _test_fixed_ips_for_port(self, context, network_id, fixed_ips,
        ...
        for fixed in fixed_ips:
            # subnets是待创建Port所关联的Network中的所有关联的Subnet
            subnet = self._get_subnet_for_fixed_ip(context, fixed, subnets)

            is_auto_addr_subnet = ipv6_utils.is_auto_address_subnet(subnet)
            # 下面的if/else是传入参数fixed_ip进行合法性判断,如果合法,就保存下来
            if ('ip_address' in fixed and
                ...
                if (device_owner in constants.ROUTER_INTERFACE_OWNERS_SNAT or
                    ...
                    fixed_ip_list.append({'subnet_id': subnet['id']})
        # 最后一步,判断所有合法的fixed_ip(fixed_ip_list)的数量是否超过一个Port所
        # 能绑定的IP地址的最大数量
