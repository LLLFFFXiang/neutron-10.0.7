...
class NeutronDbPluginV2(db_base_plugin_common.DbBasePluginCommon,
    ...
    def create_network_db(self, context, network):
        ...
        # with context.session.begin表示启动一个数据库事务:begin transaction
        with context.session.begin(subtransactions=True):
            ...
            # models_v2.Network是数据表networks的模型
            network = models_v2.Network(**args)
            # external对应Network模型中的shared字段
            if n['shared']:
                # rbac_db.NetworkRBAC是数据表networkrbacs的模型
                entry = rbac_db.NetworkRBAC(
                    network=network, action='access_as_shared',
                    target_tenant='*', tenant_id=network['tenant_id'])
                # 将entry存入表networkrbacs
    ...
    # class NeutronDbPluginV2是class Ml2Plugin的弗雷
    def _update_router_gw_ports(self, context, network, subnet):
        # l3plugin就是classL3RouterPlugin
        l3plugin = directory.get_plugin(constants.L3)
        if l3plugin:
            # 此处的Network是Router外部网关网络
            # 查询这个Network关联的外部网关端口
            gw_ports = self._get_router_gw_ports_by_network(context,
                    network['id'])
            # 通过外部网关端口,查询这个端口关联的Router
            # device_id是Port的一个属性
            # 使用这个Port的设备Id,比如一个服务实例或者一个虚拟路由器
            router_ids = [p['device_id'] for p in gw_ports]
            # 针对每一个Router,更新它的外部网关端口信息
    ...
    def _create_subnet_precommit(self, context, subnet):
        ...
        # else的意思，就是没有subnetpool_id
        else:
            # 在else(没有subnetpool_id)的条件下,再加上没有cidr,非法
            if not has_cidr:
