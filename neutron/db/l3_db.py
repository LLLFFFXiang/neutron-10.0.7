...
class L3_NAT_dbonly_mixin(l3.RouterPluginBase,
    ...
    def _update_gw_for_create_router(self, context, gw_info, router_id):
        # 首先判断gw_info不能为空,也即传入参数中是否包含了外部网关信息
        if gw_info:
            # 通过routers_id从表routers中获取一条表记录
            router_db = self._get_router(context, router_id)
            # 调用_update_router_gw_info函数
            self._update_router_gw_info(context, router_id,
                                        gw_info, router=router_db)

    @db_api.retry_if_session_inactive()
    def create_router(self, context, router):
        # router['router']是创建Router时传入的参数
        # gw_info是router['router']中的外部网关信息
        r = router['router']
        gw_info = r.pop(EXTERNAL_GW_INFO, None)
        # 以下三个create delete update_gw 涉及functools.partial算子
        ...
        update_gw = functools.partial(self._update_gw_for_create_router,
                                      context, gw_info)
        # 调用common_db_mixin.safe_creation,创建Router
    ...
    def _create_router_gw_port(self, context, router, network_id, ext_ips):
        ...
        port_data = {'tenant_id': '',  # intentionally not set
                     ...
                     'admin_state_up': True,
                     'name': ''}
        # 调用p_utils.create_port函数,
        # self._core_plugin就是前面的ML2 Plugin
        gw_port = p_utils.create_port(self._core_plugin,
                                      context.elevated(), {'port': port_data})
        # 相关数据存储在数据库表routers routerports中
        ...
        with p_utils.delete_port_on_error(self._core_plugin,
            ...
            with context.session.begin(subtransactions=True):
                # 上一步是调用ML2 Plugin的create_port函数,创建了外部网关端口
                # 这里在调用ML2 Plugin的_get_port函数,获取该端口
                router.gw_port = self._core_plugin._get_port(
                    context.elevated(), gw_port['id'])
                # l3_models.RouterPort对应表routerports
                router_port = l3_models.RouterPort(
                    router_id=router.id,
                    port_id=gw_port['id'],
                    port_type=DEVICE_OWNER_ROUTER_GW
                )
                # 相关数据存储在数据表routers routerports中
    ...
    def _create_gw_port(self, context, router_id, router, new_network_id,
        ...
        if new_valid_gw_port_attachment:
            ...
            # 继续调用 _create_router_gw_port 函数
    ...
    def delete_router(self, context, id):
        ...
        # 删除Router的外部网关端口
        # 清除表routerports中的相关数据 调用ML2 Plugin删除该外部网关端口
        self._delete_current_gw_port(context, id, router, None)
        # 获取所有关联到该Router上的所有端口,并删除
    ...
    # _check_router_port函数是校验端口的device_id fixed_ips
    # 参数device_id = router.id,参见函数_validate_router_port_info
    def _check_router_port(self, context, port_id, device_id):
        ...
        """
        # 从ML2 Plugin中通过port_id再获取Port（从数据库中再读取数据）
        port = self._core_plugin.get_port(context, port_id)
        if port['device_id'] != device_id:
            raise n_exc.PortInUse(net_id=port['network_id'],
                                  port_id=port['id'],
                                  device_id=port['device_id'])
        # 判断端口是否有'fixed_ips'。如果没有，这个端口绑定到Router上没有意义
    ...
    def _validate_router_port_info(self, context, router, port_id):
        with db_api.autonested_transaction(context.session):
            ...
            # 第3个校验为关于IPΑ额限制，端口只能关联一个IPv4 Subnet
    ...
    # 调用ML2 Plugin,更新Port的属性:device_id device_owner
    ...
    def _add_interface_by_subnet(self, context, router, subnet_id, owner):
        # 从ML2 Plugin中获取Subnet
        subnet = self._core_plugin.get_subnet(context, subnet_id)
        # 如果该Subnet没有'gateway_ip'，则非法
        ...
        port_data = {'tenant_id': subnet['tenant_id'],
                     ...
                     'device_owner': owner,
                     'name': ''}
        # 本质上是调用ML2 Plugin的接口create_port，新增一个接口
    ...
    def add_router_interface(self, context, router_id, interface_info):
        # 根据router_id获取router对象
        router = self._get_router(context, router_id)
        # 根据传入的参数interface_info,判断是通过增加Port还是通过增加Subnet
        # 来增加Router Interface.因为传入的参数,或者是port_id,或者是subnet_id
        ...
        # 如果传入参数是port_id,调用函数:_add_interface_by_port
        if add_by_port:
        ...
        # 如果传入参数是subnet_id,调用函数:_add_interface_by_subnet
        else:
        ...
        if new_router_intf:
            with mgr:
                self._notify_attaching_interface(context, router_db=router,
                                                 port=port,
                                                 interface_info=interface_info)
                # 在表routerports中添加一条记录,标识Port与Router之间的关系
                with context.session.begin(subtransactions=True):
                    router_port = l3_models.RouterPort(
                        port_id=port['id'],
                        router_id=router.id,
                ...
                # 调用ML2 Plugin update_port,修改端口的device_id device_owner属性
                self._core_plugin.update_port(
...
class L3_NAT_db_mixin(L3_NAT_dbonly_mixin, L3RpcNotifierMixin):
    ...
    def add_router_interface(self, context, router_id, interface_info):
        # 调用父类class L3_NAT_dbonly_mixin的add_router_interface
