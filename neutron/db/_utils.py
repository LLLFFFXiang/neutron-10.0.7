...
# safe_creation是一个独立的函数,不是某一个class的成员函数
def safe_creation(context, create_fn, delete_fn, create_bindings,
    ...
    with cm:
        # create_fn就是上一段代码传入的functools.partial算子create
        # 本质上是class L3_NAT_dbonly_mixin的_create_router_db函数
        obj = create_fn()
        try:
            # create_bindings就是上一段代码传入的functools.partial算子
            # update_gw本质上是class L3_NAT_dbonly_mixin的
            # _update_gw_for_create_router函数
            value = create_bindings(obj['id'])
        except Exception:
            with excutils.save_and_reraise_exception():
                try:
                    # delete_fn就是上一段代码传入的functools.partial算子delete
                    # 本质上就是class L3_NAT_dbonly_mixin的delete_router函数
                    delete_fn(obj['id'])
