...
class WorkerService(neutron_worker.NeutronWorker):
    ...
    def start(self):
        ...
        # 这里的self._service,就是class Server的实例，
        # 所以，self.service.pool就是class Server的成员变量
        # class Server调用class WorkService的函数start，
        # 而class WorkService的函数start又反过来调用class Server的pool
        # 这样的代码调用绕来绕去，比较混乱

        # 在协程中启动self._service._run这个函数
        # self._application, dup_sock都是_run函数的参数
...
class Server(object):
    ...
    def _get_socket(self, host, port, backlog):
        """
        按照普通Socket编程方法，构建一个Socket对象。观点关注host（代表ServerIP）
        和port（代表Server端口号）两个参数。其余代码仅仅是列在这里
        """
        bind_addr = (host, port)
    ...
    def start(self, application, port, host='0.0.0.0', workers=0):
        ...
        self._socket = self._get_socket(self._host,
                                        self._port,
                                        backlog=backlog)
        # 调用_launch函数，启动web server
        self._launch(application, workers)

    # workers的数值，决定启动web server的方式
    def _launch(self, application, workers=0):
        # 这里把self（也就是class Server的实例）作为参数传递给WorkerService
        service = WorkerService(self, application, self.disable_ssl, workers)
        # 如果workers < 1，则在同一个进程中 Web server
        if workers < 1:
            ...
            service.start()
            systemd.notify_once()
        # 如果workers >= 1，则新起n个进程，在每一个新的进程中再 Web server
        # 在这里的n = workers
        else:
