...
class TypeManager(stevedore.named.NamedExtensionManager):
    ...
    def create_network_segments(self, context, network, tenant_id):
        ...
        # 从传入的参数network中获取segments,
        # 穿UR参数network是指用户创建网络的RESTful API传入的参数
        segments = self._process_provider_create(network)
        session = context.session
        with session.begin(subtransactions=True):
            network_id = network['id']
            # 如果传入参数中有segments,则说明:
            # (1)用户具备管理员权限
            # (2)创建的是运营商扩展网络
            if segments:
                for segment_index, segment in enumerate(segments):
                    # self.reserve_provider_segment函数调用的是
                    # Type Driver 的reserve_provider_segment函数,意思是
                    # 将这个segment分配下来
                    segment = self.reserve_provider_segment(
                        context, segment)
                    # _add_network_segment的意思是将segment信息
                    # 存入表networksegments
                    self._add_network_segment(context, network_id, segment,
                                              segment_index)
            # 如果创建的是Router外部网关网络,并且配置了网络类型
            elif (cfg.CONF.ml2.external_network_type and
                  self._get_attribute(network, external_net.EXTERNAL)):
                # self._allocate_ext_net_segment函数在7.1.2节介绍过,
                # 简单地说,就是调用Type Driver分配Segment
                segment = self._allocate_ext_net_segment(context)
                # 将分配的Segment存入表networksegments
                self._add_network_segment(context, network_id, segment)
            else:
                # self._allocate_tenant_net_segment函数在7.1.2节介绍过,
                # 简单地说,就是调用Type Driver分配Segment
    ...
    def _allocate_segment(self, context, network_type):
        # 根据网络类型,选择对应的Type Driver
        driver = self.drivers.get(network_type)
        # 忽略下面代码中的if/else,这个细节不是重点,只需理解为
        # 两者都是调用Type Driver中的接口:allocate_tenant_segment
        if isinstance(driver.obj, api.TypeDriver):
            return driver.obj.allocate_tenant_segment(context.session)
        else:
            return driver.obj.allocate_tenant_segment(context)

    # 为租户网络分配网络类型和Segment ID
    def _allocate_tenant_net_segment(self, context):
        # self.tenant_network_types是一个数组,它的值就是前面所说的配置文件配置的
        for network_type in self.tenant_network_types:
            # 调用self._allocate_segment函数进行分配
            segment = self._allocate_segment(context, network_type)
            # 只要分配成功,就返回,不管self.tenant_network_types后面还有多少值
            if segment:
                return segment
        # 全都没有分配成功,则抛异常
        raise exc.NoNetworkAvailable()

    # 为Router外部网关网络(运营商网络的一种)分陪网络类型和Segment ID
    def _allocate_ext_net_segment(self, context):
        # Router外部网关网络类型,来源于配置文件
        # 与租户网络的分配调用的是同一个函数
...
class MechanismManager(stevedore.named.NamedExtensionManager):
    ...
    def bind_port(self, context):
        ...
        # 第一次调用，level=0
        # 重点是调用_bind_port_level函数
        # 参数0,表示binding_level=0,因为是第一次绑定,代表绑定顶层Segment
    ...
    # segments_to_bind 就是待绑定的Segment列表.这些Segmetn是并列关系
    # (分层关系的segment在后面会介绍)
        ...
        if level == MAX_BINDING_LEVELS:
            LOG.error(_LE("Exceeded maximum binding levels attempting to bind "
                      ...
                      {'port': context.current['id'],
                       'host': context.host})
            return False
        # for循环每一个Mechanism Driver
        for driver in self.ordered_mech_drivers:
            if not self._check_driver_to_bind(driver, segments_to_bind,
                                              context._binding_levels):
                continue
            try:
                # 将segments_to_bind赋值给context
                # context的类型就是class PortContext
                context._prepare_to_bind(segments_to_bind)
                '''
                    driver.obj.bind_port就是调用Mechanism Driver
                    # 的bind_port函数。bind_port函数本身没有什么特殊的,
                    可以理解为做两件事情:
                    (1) 判断这个Mechanism Driver是否合适(与厂商实现机制有关)
                    (2) 如果合适,则将_new_bound_segment、_next_segments_to_bind
                    赋值给context
                '''
                driver.obj.bind_port(context)
                # 前文介绍过，如果segment有效，说明当前的driver是合适的
                segment = context._new_bound_segment
                # 如果Segment有值，说明当前Mechanism Driver是合适的
                if segment:
                    '''
                        重点关注models.PortBindingLevel，这里面的内容将来要被存储到
                        数据库表ml2_port_bindings中。
                        context中还有其他字段，与厂商实现机制有关，也会被存入
                        数据库表ml2_port_bindings中。
                        
                        如果当前Mechanism Driver是合适的，则跳出for循环，
                        不在判断其他Driver
                    '''
                    ...
                    # 如果有下一级的segments需要绑定，那么就递归调用
                    # self._bind_port_level，继续绑定
                    if next_segments:
                        # Continue binding another level.
                        # 这里是递归调用，继续绑定，注意：level+1
