...
class Ml2Plugin(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def __init__(self):
        ...
        # 订阅PROVISIONING_COMPLETE消息的函数是_port_provisioned
    ...
    def _port_provisioned(self, rtype, event, trigger, context, object_id,
        ...
        if port.port_binding.vif_type in (portbindings.VIF_TYPE_BINDING_FAILED,
            ...
            if provisioning_blocks.is_object_blocked(context, port_id,
                ...
                return
        # 修改端口状态为const.PORT_STATUS_ACTIVE('ACTIVE')
    ...
    # 注意这个函数名:start_rpc_listeners,下文还会涉及这个函数
    @log_helpers.log_method_call
    def start_rpc_listeners(self):
        """Start the RPC loop to let the plugin communicate with agents."""
        self._setup_rpc()
        self.topic = topics.PLUGIN
        # 以下代码就是创建一个RPC Consumer
    ...
    # 注意这个函数名:start_rpc_state_reports_listener,下文还会涉及这个函数
    ...
    # self._bind_port_if_needed函数调用self._attempt_binding
    # self._attempt_binding函数调用self._bind_port
    def _bind_port(self, orig_context):
        ...
        # new_context的类型是class PortContext
        ...
        # result.
        # mechanism_manager的类型是class MechanismManager
    ...
    def _create_network_db(self, context, network):
        ...
        with session.begin(subtransactions=True):
            ...
            self.extension_manager.process_create_network(context, net_data,
                                                          result)
            # 第二步
            self._process_l3_Fcreate(context, result, net_data)
    ...
    def create_network(self, context, network):
        ...
        # registry.notify函数就是Callbacks Module机制的一部分
        # 资源类型 = 'network'，事件类型 = 'after_create'
        registry.notify(resources.NETWORK, events.AFTER_CREATE, self, **kwargs)
        try:
            # 如前文Mechanism Driver的介绍,这个函数最终会调用一个空函数
            # 因此,这是一个假的主要步骤.
    ...
    def _setup_dhcp_agent_provisioning_component(self, context, port):
        ...
        if (db.is_dhcp_active_on_any_subnet(context, subnet_ids) and
            ...
            # 向表provisioningblocks插入一条记录,entity='DHCP'
    ...
    def _create_port_db(self, context, port):
        ...
        with session.begin(subtransactions=True):
            ...
            '''
                真心觉得Neutron的函数名字取得不咋地
                一会是_create_port_db,一会是create_port_db
                这是属于毫无必要地增加代码复杂度
                这个函数的功能是:分配Port的IP地址,相关信息存入数据库表:
                ports ipallocations
            '''
            ...
            # 以下代码与Port的allowed_address_pairs extra_dhcp_opts等属性相关
            ...
            self._process_port_create_extra_dhcp_opts(context, result,
                                                      dhcp_opts)
            # 与Provisioning Block机制的相关代码就是调用这两个函数
    ...
    # 调用bind_port函数的源头,在classMlPlugin的函数create_port
    @utils.transaction_guard
    @db_api.retry_if_session_inactive()
    def create_port(self, context, port):
        ...
        try:
            # _bind_port_if_needed会调用bind_port函数
            # 绑定端口
