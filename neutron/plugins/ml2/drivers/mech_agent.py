...
class AgentMechanismDriverBase(api.MechanismDriver):
    ...
    def _insert_provisioning_block(self, context):
        ...
        if context.host_agents(self.agent_type):
            # resources.PORT = 'port',
            # provisioning_blocks.L2_AGENT_ENTITY = 'L2'
            # add_provisioning_component就是在表provisioning blocks中增加一条记录
            # 向表provisioningblocks插入一条记录,entity='L2'
    ...
    def bind_port(self, context):
        ...
        # 如果没有指定vnic_type，则取默认值portbindings.VNIC_NORMAL('normal')
        vnic_type = context.current.get(portbindings.VNIC_TYPE,
                                        portbindings.VNIC_NORMAL)
        # 如果当前Driver没有支持要绑定的vnic_type，则这个Driver无效
        if vnic_type not in self.supported_vnic_types:
        ...
        for agent in agents:
            ...
            if agent['alive']:
                # context.segments_to_bind 就是需要绑定的所有Segment
...
class SimpleAgentMechanismDriverBase(AgentMechanismDriverBase):
    ...
    # 只需关注一点，如果厂商的Driver没有给supported_vnic_types赋值，
    # 那么supported_vnic_types取默认值：portbindings.VNIC_NORMAL('normal')
    ...
    # try_to_bind_segment_for_agent在class AgentMechanismDriverBase中
    # 只是一个抽象接口，在这里才有具体的实现
    def try_to_bind_segment_for_agent(self, context, segment, agent):
        # check_segment_for_agent是真正判断Agent是否合适
        if self.check_segment_for_agent(segment, agent):
            # 如果Agent合适，则绑定这个Segment
            # 绑定的本质不过是将相关字段存入数据库表
            # 所以绑定没有什么神秘的，在这里不过是将下面三个参数设置进context而已
    ...
    def check_segment_for_agent(self, segment, agent):
        ...
        # mapping就是第4章介绍的“物理网络”，比如：
        # “bridge_mappings”: {"public":"br-ex"}
        mappings = self.get_mappings(agent)
        # allowed_network_types是Agent支持的网络类型
        ...
        # network_type是本次要绑定的Segment的网络类型
        network_type = segment[api.NETWORK_TYPE]
        # 如果绑定的Segment网络类型不在Agent支持的网络类型中，则该Agent不合适
        ...
        # 如果网络类型是Flat、VLAN，如果要绑定的Segment的物理网络
        # 与Agent配置的物理网络不一致，则该Agent不合适
