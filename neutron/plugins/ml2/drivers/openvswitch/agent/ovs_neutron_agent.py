...
class OVSNeutronAgent(sg_rpc.SecurityGroupAgentRpcCallbackMixin,
    ...
    def __init__(self, bridge_classes, conf=None):
        ...
        # 创建三类关键bridge的class，bridge_classes定义了他们的class name
        # functools.partial是一个算子
        ...
        # 构建br-int，integ_br是class OVSIntegrationBridge构造函数的参数
        # 创建class OVSIntegrationBridge的对象实例
    ...
    def setup_rpc(self):
        ...
        # 调用agent_rpc.create_consumers，创建RPC Consumer
        # [self] RPC处理对象，就是自己（class OVSNeutronAgent的实例）
    ...
    def provision_local_vlan(self, net_uuid, network_type, physical_network,
        ...
        # 下面的代码讲的就是内外VID在不同类型Bridge上的部署：
        # 如果是Tunnel类型网络，则调用self.tun_br.provision_local_vlan函数
            ...
            if self.enable_tunneling:
                ...
                LOG.error(_LE("Cannot provision %(network_type)s network for "
                          ...
                          {'network_type': network_type,
                           'net_uuid': net_uuid})
        # 如果是FLAT类型网络，则调用self._local_vlan_for_flat函数
        ...
        # 如果是VLAN类型网络，则调用self._local_vlan_for_flat函数
    ...
    def port_bound(self, port, net_uuid,
        ...
        '''
        # 调用self.provision_local_vlan函数
        # 就是在Bridge上部署内外VID转换的流表表项
        ...
        # vlan_mapping这个数据结构，就是存储内外VID映射的数据结构
        ...
        # 将内外VID映射的规则（vlan_mapping）存储在br-int的Port表中
        ...
        for port_detail in need_binding_ports:
            ...
            if self.prevent_arp_spoofing:
                self.setup_arp_spoofing_protection(self.int_br,
                                                   port, port_detail)
            # cur_tag 就是端口当前配置的tag(VLAN ID) lvm.vlan就是端口理论上的Tag
            # 如果两个不相等，则应该重新配置端口的tag，使其等于lvm.vlan
            if cur_tag != lvm.vlan:
                # 设置该端口的Tag(等于lvm.vlam)
                self.int_br.set_db_attribute(
    ...
    def setup_tunnel_br(self, tun_br_name=None):
        ...
        '''
        # 创建class OVSTunnelBridge的实例，并设置bridge name等属性
        if not self.tun_br:
    ...
    def setup_physical_bridges(self, bridge_mappings):
        ...
        # 针对bridge_mappings中的每一个Bridge，做Bridge的初始化工作
        for physical_network, bridge in six.iteritems(bridge_mappings):
            ...
            # 如果bridge_mappings中的bridge name不在已经创建好的Bridge中
            if bridge not in ovs_bridges:
                ...
                # 则退出
                sys.exit(1)
            # 创建 class OVSPhysicalBridge实例，设置bridge name等属性
            ...
            # 调用br.create，并不是真的为了创建 一个Bridge
            # 而是为了设置datapath type，因为命令行是：
    ...
    def tunnel_sync(self):
        ...
        try:
            for tunnel_type in self.tunnel_types:
                # 发送RPC消息通知Neutron Server自身的Tunnel信息
    ...
    def rpc_loop(self, polling_manager=None):
        # polling_manager的目的，就是前文介绍的
        # 对br-tun进行轮询，发现port的变化（增加的，删除的，变化的）
        # 对于polling_manager的细节，可以忽略
        ...
        while self._check_and_handle_signal():
            ...
            # 向Neutron Server通报自己的Tunnel信息
            # tunnel_sync的初始值是True，但是tunnel_sync的返回值是False，
            # 如果它运行正确，即OVS Agent只会向Neutron Server通报
            # 1 次其自身的Tunnel信息
    ...
    def daemon_loop(self):
        ...
        # 捕捉信号SIGTERM,处理函数是self._handle_sigterm
        signal.signal(signal.SIGTERM, self._handle_sigterm)
        if hasattr(signal, 'SIGHUP'):
            # 捕捉信号SIGHUP，处理函数是self._handle_sighup
    ...
    def _handle_sigterm(self, signum, frame):
        # self.catch_sigterm，正是_check_and_handle_signal的判断条件
    ...
    def _handle_sighup(self, signum, frame):
        # self.catch_sighup，正是_check_and_handle_signal的判断条件
        self.catch_sighup = True

    # self.run_daemon_loop的初始值为True，意思不退出while循环
    def _check_and_handle_signal(self):
        ...
        if self.catch_sighup:
            ...
            # 这里没有设置self.run_daemon_loop的值，意思是不退出循环
            self.catch_sighup = False
