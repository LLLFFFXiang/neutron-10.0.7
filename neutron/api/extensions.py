...
class ExtensionMiddleware(base.ConfigurableMiddleware):
    ...
    def __init__(self, application,
                 ext_mgr=None):
        # plugin_aware_extension_middleware_factory方法中
        # ext_mgr就是 PluginAwareExtensionManager.get_instance()
        # 代码ext_mgr = PluginAwareExtensionManager.get_instance()
        self.ext_mgr = (ext_mgr
                        or ExtensionManager(get_extensions_path()))
        mapper = routes.Mapper()

        # extended resources
        # 从self.ext_mgr中获取全部资源
        # 等价于一下代码（伪代码）
        # for ext_service in all Extension Services
        #     resource = ext_service.get_resources()
    ...
    def __call__(self, req):
        ...
        # 跟Core Service一样，返回一个RoutesMiddleware对象
        return self._router
    ...
    def _dispatch(req):
        ...
        # 重点在这里，如果没有找到，则return req.environ['extended.app']
        # req.environ['extended.app']就是Core Service的WSGI Application
        # 就是class APIRouter的实例对象
        # 与class Router（class APIRouter的父类）的_dispatch相比，除了if，
        # 其他一模一样
        # 这就是一个Request得首先达到app_ext,app_ext经过相应的处理，才会交由app_v2.0
        if not match:
...
def plugin_aware_extension_middleware_factory(global_config, **local_config):
    """Paste factory."""
    # _factory是一个内嵌函数，参数app就是Core Service的WSGI Application的实例，
...
class ResourceExtension(object):
    ...
    def __init__(self, collection, controller, parent=None, path_prefix="",
        ...
        # 就是RESOURCE_ATTRIBUTE_MAP分解后的每个资源的字段属性列表
