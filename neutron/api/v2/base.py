...
class Controller(object):
    ...
    # 当资源为network时，等价于return self.plugin.create_network(request.context,
    # **kwargs)
    ...
    def _create(self, request, body, **kwargs):
        ...
        # 注意，如果资源是network，函数名变为“create_network”
        # 如果资源是port，则函数名变为“create_port”
        # 这里，假设的资源是network，所以获取的函数名是“create_network”
        ...
        def do_create(body, bulk=False, emulated=False):
            ...
            if bulk and not emulated:
                ...
                # 从插件（self._plugin）中，获取函数“create_network”
                # self._plugin就是class Ml2Plugin的对象实例
            ...
            try:
                if emulated:
                    ...
                    # 执行刚刚获取到的函数：self._plugin.create_network
