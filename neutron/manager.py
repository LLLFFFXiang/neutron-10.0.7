...
class NeutronManager(object):
    ...
    # 加载Core Service 的plugin
    def _get_plugin_instance(self, namespace, plugin_provider):
        # 调用load_class_for_provider,获取Plugin的class
    ...
    def _get_default_service_plugins(self):
        ...
        # constants.DEFAULT_SERVICE_PLUGINS是一个常量
        return constants.DEFAULT_SERVICE_PLUGINS.keys()

    # 加载Extension Services的Plugin
    def _load_service_plugins(self):
        ...
        """
        # plugin_providers配置在配置文件中
        plugin_providers = cfg.CONF.service_plugins
        # _get_default_service_plugins()是获取默认的plugin_provider
        # 这些plugin不需要配置文件配置
...
def init():
    ...
    # 加载插件(core plugin + extension services plugin)
    # 代码第一次执行到这里,directory.get_plugins()肯定没有plugin
    # 因为还没有加载plugin
    if not directory.get_plugins():
        # 如果把代码当做一篇文章的话,这代码写得让人莫名其妙
        # 如果没有插件,那么NeutronManager.get_instance(),简直就是驴唇不对马嘴
