...
class LinuxInterfaceDriver(object):
    ...
    def init_l3(self, device_name, ip_cidrs, namespace=None,
        ...
        """
        # preserve_ips是需要保留的IP地址列表
        preserve_ips = preserve_ips or []
        # 不用在意device的细节，它的本质就是构建并执行相应的Linux命令行
        ...
        # 以下的代码做了两件事：
        # 1. 将需要新增的cidr存储在cidrs数组中
        # 2. 将需要删除的cidr存储在remove_ips数组中
        ...
        # 删除端口上的不需要的IP地址
        # 假设namespace = qrouter - 12345
        # 对应的命令行是：ip netns exec qrouter-12345 ip addr del ...
        ...
        # 在端口上添加需要新增的IP地址
        # 假设namespace = qrouter-12345
        # 对应的命令行是：ip netns exec qrouter-12345 ip addr add
    ...
    def init_router_port(self,
        ...
        # 首先调用self.init_l3，重新配置端口的IP地址
        self.init_l3(device_name=device_name,
                     ip_cidrs=ip_cidrs,
                     namespace=namespace,
                     preserve_ips=preserve_ips or [],
                     clean_connections=clean_connections)
        # 不用在意device的细节，本质就是构建并执行相应的Linux命令行
        device = ip_lib.IPDevice(device_name, namespace=namespace)

        # Manage on-link routes (routes without an associated address)
        # 下面的算法逻辑，与self.init_l3函数类似
        # 1. 通过传入的参数，构建new_onlink_cidrs
        # 注意：extra_subnets的值可能为空
        new_onlink_cidrs = set(s['cidr'] for s in extra_subnets or [])
        # 2. 通过查询已有的路由表，构建existing_onlink_cidrs，
        # 对应的命令行是ip netns exec qrouter-12345 ip router show ...
        ...
        # 天啊及需要新增的路由表项
        # 对应的命令行是ip netns exec qrouter-12345 ip router add...
        for route in new_onlink_cidrs - existing_onlink_cidrs:
            LOG.debug("adding onlink route(%s)", route)
            device.route.add_onlink_route(route)
        # 删除需要新增的路由表项
        # 对应的命令行是ip netns exec qrouter-12345 ip router del...
    ...
    def plug(self, network_id, port_id, device_name, mac_address,
             bridge=None, namespace=None, prefix=None, mtu=None):
        # 首先通过device_name判断device是否存在
        # device_name就是interface name(Router 与 Bridge对接的接口名称)
        if not ip_lib.device_exists(device_name,
                                    namespace=namespace):
            try:
                # 如果不存在，则调用plug_new函数，创建一个新的接口，
                # plug_new被class OVSInterfaceDriver重载了
...
class OVSInterfaceDriver(LinuxInterfaceDriver):
    ...
    def plug_new(self, network_id, port_id, device_name, mac_address,
        ...
        # 参数Bridge，指的是OVS Bridge 的名称，而不是一个网桥（Bridge）
        # 像python 这样的代码，不需要指明变量类型，只有一个变量名，如果变量的名字
        # 比较模糊，这对于代码的阅读会造成很大的困扰。Neutron的代码，就像这样的问题很多
        # 如果参数没有传递Bridge名
        if not bridge:
            # 那么Bridge名就咦配置文件为准（br-int）
            bridge = self.conf.ovs_integration_bridge
        # 判断这个Bridge是否存在，如果不存在，则报异常
        # check_bridge_exists函数调用的函数是ip_lib.device_exists(bridge)
        # 也就是说，是通过Linux命令行来判断
        self.check_bridge_exists(bridge)
        # ip_lib这个模块，可以简单理解为对Linux IP相关的命令行进行封装
        ip = ip_lib.IPWrapper()
        # _get_tap_name函数就是对构建接口名称
        # 如果是tap类型，则tap_name=device_name
        tap_name = self._get_tap_name(device_name, prefix)
        # 如果接口类型是veth pair，为了易于描述和理解，忽略这个分支
        if self.conf.ovs_use_veth:
            ...
            # 我们只关注接口类型是tap的这个分支
            # ip.devidce仍然可以简单理解为Linux IP命令行的封装
            ns_dev = ip.device(device_name)
        # internal = True
        internal = not self.conf.ovs_use_veth
        # 本质是调用ovs-vsctl命令行，在Bridge上增加一个Port
        ...
        # 如果不使用veth pair，并且有namespace的话
        # 把前面ovs-vsctl命令行增加的端口，添加到namespace中
        if not self.conf.ovs_use_veth and namespace:
            # 下面两句代码，本质都有调用Linux命令行
