...
class L3NATAgent(ha.AgentMixin,
    ...
    def routers_updated(self, context, routers):
        ...
        if routers:
            ...
            for id in routers:
                # 构建元素：class RouterUpdate的对象实例
                update = queue.RouterUpdate(id, queue.PRIORITY_RPC)
                # 将class RouterUpdate的对象实例（update）存入数组 _queue
                self._queue.add(update)
        ...
        if (target_ex_net_id and ex_net_id and ex_net_id != target_ex_net_id):
            ...
            if ex_net_id != self._fetch_external_net_id(force=True):
                raise n_exc.RouterNotCompatibleWithAgent(
                    router_id=router['id'])
        # Router变更消息中并没有指明这个Router是新增消息还是修改消息
        # 如果这个Router ID不在当前存储的Router ID范围内，则认为是新增
    ...
    # 一共m个协程，每个协程都会执行这个函数（_process_router_update）
    def _process_router_update(self):
        # while (True): 这段代码会被死循环执行，为了直观表现循环调用
        # self._queue,就是前面介绍的class RouterProcessingQueue的实例
        # self._queue.each_update_to_next_router，神一样的接口
        # 保障了Router变更消息处理机制
        for rp, update in self._queue.each_update_to_next_router():
            # 下面的代码不用关注细节，就当做是获取到变更信息以后，用命令行配置Router
            # 处理Router变更消息（update）
            ...
            if update.action != queue.DELETE_ROUTER and not router:
                try:
                    # 变更消息的类型不是删除，但是变更消息中没有带Router的具体信息
                    # 重新从Neutron Server那边获取Router信息
                    # 但是这个Router信息的时间戳已经不是当初发生变更消息的时间戳
                    # 而是当前重新获取Router消息中的时间戳
                ...
                if routers:
                    router = routers[0]
            # 变更消息中只有Router ID，而无Router真正内容，意味着要删除这个Router，
            # 当然实际代码中，还有其他判断逻辑，被忽略掉了
            if not router:
                ...
                if not removed:
                    ...
                    # rp就是class ExclusiveRouterProcessor的实例
                    rp.fetched_and_processed(update.timestamp)
                LOG.debug("Finished a router update for %s", update.id)
                continue

            try:
                # 主干代码 _process_router_if_compatible
            ...
            LOG.debug("Finished a router update for %s", update.id)
            # rp就是class ExclusiveRouterPRocessor的实例
            # 注意，此时的时间戳已经是当前重新获取Router消息中的时间戳了
            # rp.fetched_and_processed，是为了保证过期的Router变更消息不在处理
            rp.fetched_and_processed(update.timestamp)

    def _process_routers_loop(self):
        LOG.debug("Starting _process_routers_loop")
        pool = eventlet.GreenPool(size=8)
        # 一个死循环（while True），协程中运行_process_router_update函数
        while True:
...
class L3NATAgentWithStateReport(L3NATAgent):
    ...
    def __init__(self, host, conf=None):
        ...
        # 报告状态周期，配置在配置文件 etc/neutron.conf, report_interval = 30(秒)
        report_interval = self.conf.AGENT.report_interval
        if report_interval:
            # 周期性的执行self._report_state函数，就是周期性报告L3 Agent的状态
    ...
    def after_start(self):
        # 在协程里启动self._process_routers_loop
        eventlet.spawn_n(self._process_routers_loop)
        LOG.info(_LI("L3 agent started"))
