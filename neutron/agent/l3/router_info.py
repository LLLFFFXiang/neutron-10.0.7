...
class RouterInfo(object):
    ...
    def initialize(self, process_monitor):
        ...
        self.radvd = ra.DaemonMonitor(self.router_id,
                                      ...
                                      self.agent_conf)
        # self.router_namespace是在__init__函数中构建的
    ...
    # old_routes = self.routes，保存的路由表信息
    # new_routes = self.router['routes']，用户新传入的路由表信息
    def routes_updated(self, old_routes, new_routes):
        # 新旧两个路由表比较，获取：待新增的路由表项（adds）、待删除的路由表项（remove）
        adds, removes = helpers.diff_list_of_dict(old_routes,
                                                  new_routes)
        for route in adds:
            LOG.debug("Added route entry is '%s'", route)
            # remove replaced route from deleted route
            # 待删除路由表项与待新增路由表项比较，如果两者目的地相同，则不认为是待删除，
            # 而是更新，所以要从待删除路由表项中移除这些表项
            for del_route in removes:
                if route['destination'] == del_route['destination']:
                    removes.remove(del_route)
            #replace success even if there is no existing route
            # 调用ip route replace 命令，更新路由表项（包括"新增"的和变化的）
            self.update_routing_table('replace', route)
        for route in removes:
            LOG.debug("Removed route entry is '%s'", route)
            # 调用ip route delete命令，删除待删除的路由表项
    ...
    def process_floating_ip_nat_rules(self):
        ...
        for fip in floating_ips:
            ...
            # SNAT/DNAT就是fixed与fip_ip互相映射
            fixed = fip['fixed_ip_address']
            fip_ip = fip['floating_ip_address']
            # 构建rule和chain（chain是iptables的概念）
    ...
    def process_snat_dnat_for_fip(self):
        try:
            # 调用process_floating_ip_nat_rules
    ...
    def _internal_network_added(self, ns_name, network_id, port_id,
        ...
        LOG.debug("adding internal network: prefix(%s), port(%s)",
                  prefix, port_id)
        # 前面分析过，就是创建Router与OVS Bridge之间对接的接口
        self.driver.plug(network_id, port_id, interface_name, mac_address,
                         namespace=ns_name,
                         prefix=prefix, mtu=mtu)
        # 前面分析过，就是初始化Router接口（配置接口的IP地址，配置Router的路由表）
        ip_cidrs = common_utils.fixed_ip_cidrs(fixed_ips)
        self.driver.init_router_port(
            interface_name, ip_cidrs, namespace=ns_name)
        # 下面的代码是发送免费ARP（细节不用关注），已验证IP地址是否重复
        for fixed_ip in fixed_ips:
    ...
    def internal_network_added(self, port):
        ...
        # interface_name（接口名称）的格式是：'qr-' + port_id
        interface_name = self.get_internal_device_name(port_id)
        # 调用_internal_network_added
    ...
    def _process_internal_ports(self):
        # existing_port_ids表示当前为一个Router存储的内部接口
        # 是存储在self.internal_ports中，不是表示真的添加在Router上
        existing_port_ids = set(p['id'] for p in self.internal_ports)

        # current_port_ids是传入的参数（通过参数router获取）
        # 表示一个Router(路由器)当前应该具有的端口
        # 参数router是Neutron Server通过RPC传递过来的，其数据结构
        # 可以参考Router的逻辑模型，两者基本一致
        internal_ports = self.router.get(lib_constants.INTERFACE_KEY, [])
        current_port_ids = set(p['id'] for p in internal_ports
                               if p['admin_state_up'])
        # 构建需要新增的、删除的、更新的端口列表：new_ports old_ports updated_ports
        ...
        enable_ra = False
        # 如下代码，是分别针对需要新增的、删除的、更新的端口列表做处理
        for p in new_ports:
    ...
    def _plug_external_gateway(self, ex_gw_port, interface_name, ns_name):
        # self.driver就是class OVSInterfaceDriver的对象实例
    ...
    def _external_gateway_added(self, ex_gw_port, interface_name,
        ...
        # 初始化路由端口（为外部网关端口配置IP地址等）
        # self.driver就是class LinuxInterfaceDriver的对象实例
        self.driver.init_router_port(
            interface_name,
            ip_cidrs,
            namespace=ns_name,
        ...
        # 增加默认静态路由，本质上是调用了命令行 ip route replace default via ...
    ...
    def _process_external_gateway(self, ex_gw_port):
        ...
        interface_name = None
        if ex_gw_port_id:
            # 获取外部网关端口的名字（'qg-' + ex_gw_port_id）
            interface_name = self.get_external_device_name(ex_gw_port_id)
        if ex_gw_port:
            if not self.ex_gw_port:
                # 增加（创建）外部网关端口，并创建默认路由
    ...
    def process_external(self):
        ...
        try:
            with self.iptables_manager.defer_apply():
                ...
                # 处理Floating IP 的SNAT/DNAT
                self.process_snat_dnat_for_fip()

            # Once NAT rules for floating IPs are safely in place
            # configure their addresses on the external gateway port
            # 将Floating IP配置在Router的外部网关端口上
            interface_name = self.get_external_device_interface_name(
                ex_gw_port)
            fip_statuses = self.configure_fip_addresses(interface_name)
    ...
    def process(self):
        ...
        # 将ex_gw_port Floating IP 映射信息、enable_snat等信息存储下来
        # 这里体现了RouterInfo的字面含义（仅仅是存储相关信息的数据结构）
