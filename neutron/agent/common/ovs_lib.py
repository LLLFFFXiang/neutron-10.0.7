...
class BaseOVS(object):
    ...
    # 下面代码都是调用了self.ovsdb的接口
    def delete_bridge(self, bridge_name):
        # del_br返回的命令行对象，所以要调用这个对象的execute接口，以执行这个命令行
        # 两个代码结合
        # ovs-vsctl --if-exists del-br br1 假设bridge name = 'br1'
...
class OVSBridge(BaseOVS):
    ...
    # class BaseOVS的add_bridge函数，就是调用的class OVSBridge的create接口
    # 假设：bridge name = br0，datapath_type = system
    # 执行的命令
    # ovs-vsctl --may-exist add-br br0 -- set Bridge br0 datapath_type=system
    ...
    # 为一个bridge增加一个端口
    # 假设：bridge name = br0，port name =eth1
    # ovs-vsctl add-port --may-exist br0 eth1
    ...
    def do_action_flows(self, action, kwargs_list):
        ...
        # _build_flow_expr_str函数的细节不是关键，就是构建*-flows的命令参数
        # *-flows指的是add-flows del-flows
        flow_strs = [_build_flow_expr_str(kw, action) for kw in kwargs_list]
        # action = 'add'，所以命令行是add-flows
        '''
            假设bridge name = br0
            priority=2, in_port=2,是drop_port函所赋的值
            table=0,actions=drop,是install_drop函数所赋的值
            这个命令行的意思是：所有从br0的端口2进入的流都丢弃（drop）
            ovs-ofctl add-flows br0 "table=0, priority=0, in_port=2, actions=drop"
        '''
        self.run_ofctl('%s-flows' % action, ['-'], '\n'.join(flow_strs))

    # 增加一个flow（流表项）
    def add_flow(self, **kwargs):
        # 直接调用函数do_action_flows
