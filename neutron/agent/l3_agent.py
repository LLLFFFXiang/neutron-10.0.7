...
# 请注意mangaer这个参数： 'neutron.agent.l3.agent.L3NATAgentWithStateReport'
def main(manager='neutron.agent.l3.agent.L3NATAgentWithStateReport'):
    register_opts(cfg.CONF)
    common_config.init(sys.argv[1:])
    config.setup_logging()
    config.setup_privsep()
    # 在这里变量名是server，当把这个server传入service.launch(..., server)时，
    # 它的名字被称为service。
    # manager传给了neutron_service.Service
    # 创建Service
